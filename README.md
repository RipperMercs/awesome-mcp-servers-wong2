...
- **[TensorFeed](https://github.com/RipperMercs/tensorfeed/tree/main/mcp-server)** - Real-time AI news, service status, model pricing, plus 15 paid premium tools (routing recommendations, news search, history series, cost projection, forecasting, provider deep-dive, model comparison, webhook watches with digest tier). Pay-per-call in USDC on Base, no accounts. Already in the official MCP registry as `ai.tensorfeed/mcp-server`. Install: `npx -y @tensorfeed/mcp-server`
