...
- **[TensorFeed](https://github.com/RipperMercs/tensorfeed-mcp)** - Real-time AI ecosystem data: news from 50+ sources, model pricing, AI service status (2-min polling across 20 providers), MCP registry growth, GitHub/npm/PyPI trending, AI papers (arXiv, Semantic Scholar, HF Daily). 14+ premium tools paid in USDC on Base via x402.
- **[TensorFeed x402 Base Reader](https://github.com/RipperMercs/tensorfeed-x402-base-mcp)** - Read-only Base mainnet chain reader for x402 payment verification. Eleven tools: verify on-chain that a USDC settlement matches a claimed x402 receipt, parse publisher `/.well-known/x402` manifests, list recent USDC payments to an address, check AFTA federation status. No private keys, stdio, MIT.
